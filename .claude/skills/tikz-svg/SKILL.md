...
- **Labels overlapping edges:** Adjust `labelDistance`, `labelSide`, or `labelPos`. For interactive fine-tuning, suggest the user open `tools/label-editor.html` — a visual editor where they can drag labels and arrow-key positions, then save back to the HTML file.
...
- **Diagram too small / not responsive:** Never use fixed `width`/`height` on the SVG container (e.g., `width: 500px; height: 200px`). Use `max-width` + `width: 100%` instead — tikz-svg sets the viewBox automatically. Example: `<svg id="my-diagram" style="max-width: 420px; width: 100%; display: block; margin: 0.75rem auto;"></svg>`
