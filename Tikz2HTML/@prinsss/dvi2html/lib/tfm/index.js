...
var Tfm = require("./tfm");
var fontdata = require("./fonts.json");
var NO_TAG = 0;
var LIG_TAG = 1;
var LIST_TAG = 2;
var EXT_TAG = 3;
  ...
  ***/
    TFMParser.prototype.read_font_parameters = function () {
        ...
        if (this.tfm.character_coding_scheme == 'TeX math italic') {
            ...
            this.tfm.set_font_parameters(__spread(Array(7).keys()).map(function () { return stream.read_fix_word(); }));
        }
        if (this.tfm.character_coding_scheme == 'TeX math symbols') {
            // Read the additional 15 fix word parameters
            this.tfm.set_math_symbols_parameters(__spread(Array(15).keys()).map(function () { return stream.read_fix_word(); }));
        }
        if ((this.tfm.character_coding_scheme == 'TeX math extension') ||
            ...
            this.tfm.set_math_extension_parameters(__spread(Array(6).keys()).map(function () { return stream.read_fix_word(); }));
        }
    };
...
function tfmData(fontname) {
    if (fontdata[fontname]) {
        var buffer = buffer_1.Buffer.from(fontdata[fontname], 'base64');
        return buffer;
    }
